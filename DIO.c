...
{
	unsigned int device;
	unsigned int num,hi_low;
	
	int i;
	device=0;//pin3,5,7,9 is do  pin 11,13,15,17 is di
	//0xf0 pin3,5,7,9 is do      pin 11,13,15,17 is di
	...
	device=2;//pin4,6,8,10 is do pin 12,14,16,18 is di
	//0xf0 pin4,6,8,10 is do     pin 12,14,16,18 is di
	AXBoardSetIndexValue(EAPI_AX_HWMON_MAIN_DIO_DIR,device,0xF0);
	
	device=3;//pin20,22,24,26 is do  pin 28,30,32,34 is di
	...
	*/
	...
	printf("set pin 3,5,7,9 is low\n");
	for (i=0;i<4;i++)
		AXBoardSetValue(EAPI_AX_HWMON_DO0+i,0);
	for(i=0;i<4;i++)
	{
		//get pin 3,5,7,9
		EApiBoardGetValue(EAPI_AX_HWMON_DO0+i,&hi_low);
		printf("do%d,[%d]\n",i,hi_low);
	}
	...
	{
		//get pin 11,13,15,17
		EApiBoardGetValue(EAPI_AX_HWMON_DI4+i,&hi_low);	
		printf("di%d,[%d]\n",4+i,hi_low);
	}

	//set pin 19,21,23,25 is low
	for (i=0;i<4;i++)
